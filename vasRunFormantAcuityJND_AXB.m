function vasRunFormantAcuityJND_AXB()
%vasRunFormantAcuity_AXB() is an experimental paradigm for testing perceptual
%acuity to changes in vowel formants using a AXB method. This program uses a previously
...
numlines = 1;
defaultanswer = {'null', 'FA1', 'GT1', 'Diff', 'female'};
...
dirs = vasDirs(UD.project);
% Folder paths to save data files
dirs.RecFileDir = fullfile(dirs.RecData, UD.subject, UD.run);
dirs.TokenFile  = fullfile(dirs.tokenDir, UD.tokenFile, [UD.tokenFile 'vas.mat']);
...
UD.F1Steps    = GT.F1Steps;
UD.F2Steps    = GT.F2Steps;
UD.fs         = GT.fs;
...
UD.xMin = 2;           % min difference between speaker's fo and fo of stimulus in headphones
UD.xAll = -50:1:50;
...
fprintf('Starting Formant Acuity Task for %s\n', UD.subject)
%%%%%Visual Presentation
...
while (UD.stop == 0) && tr < UD.totalTrials
    ...
    UD = vasAdaptiveUpdateJNDAXB(UD, response, correct);
